...
def load_incum_table():
    ...
    incum.to_csv("clean_incumbents.csv")
...
#Data cleaned from LOP table 'candidates' to link with incumbent table, only looking at general elections for now
df = pd.read_csv("FED_1867_present.csv")
df = df[df['Election_Type'] == "General"]

#Create unique ids for both tables
...
df2.to_csv("clean_candidates.csv")
